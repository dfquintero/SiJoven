...
function getDataFromSvc(){
    console.log("all data from svc");
    $.ajax({
...
function itemsPush(elemento) {
   
    console.log("items push method a elemento:" + elemento);
    var items = [];
    ...
    $("<ul/>", {
        "id": "eventList_" + elemento, "data-role": "listview", "data-inset": "true", "data-filter": "true",
        html: items.join("")
    }).appendTo(container);
}

function itemsPushWithFilter(categoria, elemento) {
  
    console.log("items push method");
    var items = [];
...
function filterResults(categoria, elemento) {
    ...
    itemsPushWithFilter(categoria, elemento);
    console.log('finish push filtro: ' + categoria);
